...
export function OnboardingAgeScreen({ navigation }: AuthStackScreenProps<"OnboardingAge">): ReactElement {
  ...
  const [age, setAge] = useState(() => useOnboardingStore.getState().age);

  useFocusEffect(
    useCallback(() => {
      setAge(useOnboardingStore.getState().age);
    }, []),
  );
