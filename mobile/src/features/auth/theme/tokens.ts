...
export const layout = {
  statusBarPadH: 28,
  /** Added to content top padding on iOS inside `OnboardingLayout` (extra air below Dynamic Island / notch). */
  iosTopAir: 12,
   ...
   * On iOS, `OnboardingLayout` adds `iosTopAir` on top of index 0; system safe area is separate (SafeAreaView).
   */
...
export function authShellMaxWidth(windowWidth: number): number {
  if (windowWidth >= 768) {
